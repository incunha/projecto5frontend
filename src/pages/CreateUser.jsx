...
function CreateUser() {
  ...
  const handleSubmit = async (event) => {
    ...
    const response = await fetch(
      "http://localhost:8080/projecto5backend/rest/users",
      {
        method: "POST",
        headers: {
    ...
    if (response.ok) {
      alert(
        "\u{1F389} A confirmation email has been sent to the email address"
      );
    } else {
      alert("\u{1F6AB} An error occurred while creating the account");
    }
  };
