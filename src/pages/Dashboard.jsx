...
const Dashboard = () => {
  ...
  // Função para atualizar as estatísticas dos utilizadores e das tarefas
  useEffect(() => {
    if (token) {
  ...
  // Função para atualizar as estatísticas dos utilizadores e das tarefas com base na mensagem recebida
  useEffect(() => {
    if (message === "ping") {
      fetchUserStatistics(token);
      fetchTaskStatistics(token);
  ...
  const userStats = userStatistics && [
    ...
    { name: "Confirmed Users", value: userStatistics.totalConfirmedusers || 0 },
    {
      ...
      value: userStatistics.totalUnconfirmedUsers || 0,
    },
  ...
  const confirmedUsersByDate =
    userStatistics &&
    userStatistics.confirmedUsersByDate &&
    Object.entries(userStatistics.confirmedUsersByDate)
      .map(([date, count]) => ({
        date,
        count,
      }))
  ...
  const averageTasksPerUser = taskStatistics
    ? taskStatistics.averageTasksPerUser
    : 0;
  ...
  const pieDataTasks = taskStatistics
    ? Object.entries(taskStatistics.tasksByCategory).map(([name, value]) => ({
        name,
        value,
      }))
    : [];
  ...
  const tasksByCategory =
    taskStatistics && taskStatistics.tasksByCategory
    ...
    <div>
      ...
      <div style={{ display: "flex" }}>
        ...
        >
          ...
          <p>
            <strong>{t("Average tasks per user")}:</strong>{" "}
            {averageTasksPerUser}
          </p>
          <p>
            <strong>{t("Total tasks")}:</strong> {totalTasks}
          </p>
          <p>
            <strong>{t("Tasks in DONE")}:</strong> {totalDoneTasks}
          </p>
          <p>
            <strong>{t("Tasks in DOING")}:</strong> {totalDoingTasks}
          </p>
          <p>
            <strong>{t("Tasks in TO DO")}:</strong> {totalToDoTasks}
          </p>
        ...
        >
          <div>
              ...
              {userStats && userStats.length > 0 && (
                <div style={{ flex: "1 0 400px", margin: "1rem" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center" }}>
                    ...
                    <div style={{ paddingLeft: "30px", fontSize: "0.8rem" }}>
            ...
            >
              <div style={{ flex: "1 0 300px", margin: "1rem" }}>
                <h2 style={{ fontSize: "1rem" }}>{t("Task Statistics")}</h2>
                <div
